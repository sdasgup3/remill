...
typedef std::unique_ptr<cs_insn, std::function<void (cs_insn *)>> CapstoneInstructionPtr;
...
public:
  ...
  */
  ...
  virtual bool InstructionOperands(std::vector<Operand> &operand_list, const CapstoneInstructionPtr &capstone_instr) const noexcept = 0;

  /// Returns the address size, in bits
  virtual std::size_t AddressSize() const noexcept = 0;

  /// Returns the instruction category
  virtual Instruction::Category InstructionCategory(const CapstoneInstructionPtr &capstone_instr) const noexcept = 0;
};
